...
test_page.py
------------
Test cases for the ELC website using the Hybrid Selenium Framework.
Inherits from BaseTest (local Chrome WebDriver) and uses Page Object Model.
"""
...
class TestCaseClass(BaseTest):
    """Collection of UI test cases for the ELC e-commerce website."""

    logger = get_framework_logger()

    # ------------------------- SETUP & TEARDOWN -------------------------
    def setUp(self):
        """Launch the browser, load the application URL, and set timeouts."""
        try:
    ...
    # TestCase 1: Verify Newborn Gifts navigation and first product flow
    @pytest.mark.smoke
